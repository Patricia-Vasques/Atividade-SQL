...
--Tarefas já concluidas
SELECT * FROM Tarefa WHERE concluida = true;

--Tarefas que estão atrasadas
SELECT * FROM Tarefa WHERE data_prazo < CURRENT_DATE AND concluida = false;

--Contagem de tarefa por projeto
SELECT projeto_id, COUNT(*) as total_tarefas FROM Tarefa GROUP BY projeto_id;